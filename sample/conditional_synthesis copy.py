...
def main():
    ...
    # torch.save((input_motions, model_kwargs),'temp_save.pt')
    base_input_motions, base_model_kwargs=torch.load('temp_save.pt')
    # allone_list=[1,1,1,1,1]
    # needlist=[1,13, 18, 25, 35]
    allone_list=[1, 1,1,1,1]
    needlist=[1,1, 1, 1, 1]
    ...
    modify_motions[0, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[5, :, :, 0:4]
    # model_kwargs['y']['text'][0] = 'a person makes a fighting pose'
    modify_motions[1, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[9, :, :, 60:64]
    # model_kwargs['y']['text'][1] = 'a person kneels down and stands up to keep clapping'
    modify_motions[2, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[10, :, :, 60:64]
    # model_kwargs['y']['text'][2] = 'a person is running in place'
    modify_motions[3, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 50:54]
    # model_kwargs['y']['text'][3] = 'a person is balancing on his right foot'
    modify_motions[4, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[18, :, :, 50:54]
    # model_kwargs['y']['text'][4] = 'a person jumps twice and then keep clapping'
