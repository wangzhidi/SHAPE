...
def main():
    ...
    # data_dict=data.dataset.t2m_dataset.data_dict
    # motion_list = []
    # lengths_list = []
    # names_list = []
    ...
    #     lengths_list.append(value['length'])
    #     motion = value['motion']
    #     processed_motion = np.transpose(motion, (1, 0)) # (196, 263) -> (263, 196)
    #     processed_motion = np.expand_dims(processed_motion, axis=1) # (263, 196) -> (263, 1, 196)
    #     motion_list.append(processed_motion)

    # # motions = np.stack(motion_list, axis=0) 
    # lengths = np.array(lengths_list, dtype=np.int32) 
    # names = np.array(names_list) 
    # sample_list=[]
    # for sample in tqdm(motion_list):
    # # Unnormalize samples and recover XYZ *positions*
    #     sample=torch.from_numpy(np.array([sample])).float().to(dist_util.dev())
    #     n_joints = 22 if (sample.shape[1] in [263, 264]) else 21
    #     sample = sample.cpu().permute(0, 2, 3, 1)   #[5,263,1,196]-->[5,1,196,263]
    #     sample = data.dataset.t2m_dataset.inv_transform(sample).float()
    #     sample = recover_from_ric(sample, n_joints, abs_3d=args.abs_3d) #[5,1,196,22,3]
    #     sample = sample.view(-1, *sample.shape[2:]).permute(0, 2, 3, 1) # [5,22,3,196]
    #     sample_list.append(sample.cpu().numpy())
    
    # from draw_sketches import draw_sketches
    # for i in range(len(sample_list)):
    #     sample=sample_list[i][0]
    #     sample=sample.transpose(2,0,1) # (22,3,196) -> (196,22,3)
    #     draw_sketches(sample,"visualization/sketch" ,i)
    ...
    # torch.save((input_motions, model_kwargs),'temp_save_3561.pt')
    base_input_motions, base_model_kwargs=torch.load('temp_save_3310.pt')

    allone_list=[1, 1,1,1,1]
    needlist=[1,1, 1, 1, 1]
    # allone_list=[]
    # needlist=[]
    # for i in range(64):
    #     allone_list.append(i)
    #     needlist.append(i)


    args.batch_size=needlist.__len__()
    args.num_samples=needlist.__len__()
    input_motions=base_input_motions[allone_list]
    model_kwargs['y']['mask'] = base_model_kwargs['y']['mask'][allone_list]
    model_kwargs['y']['lengths'] = base_model_kwargs['y']['lengths'][allone_list]
    model_kwargs['y']['text'] = [base_model_kwargs['y']['text'][i] for i in needlist]
    model_kwargs['y']['tokens'] = [base_model_kwargs['y']['tokens'][i] for i in needlist]
    ...
    # modify input_motions, replace the middle 5 frames with the last 5 frames
    n_frames = input_motions.shape[-1]
    modify_motions = input_motions.clone()
    for i in range(input_motions.shape[0]):
        length=model_kwargs['y']['lengths'][i].item()
        # modify_motions[i, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[5, :, :, 0:4]
        modify_motions = modify_motions.to(dist_util.dev())
        
    
    modify_motions[0, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 40:44]
    model_kwargs['y']['text'][0] = 'A person is clapping.'
    modify_motions[1, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 50:54]
    model_kwargs['y']['text'][1] = 'A person is clapping.'
    modify_motions[2, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 80:84]
    model_kwargs['y']['text'][2] = 'A person is clapping.'
    modify_motions[3, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 110:114]
    model_kwargs['y']['text'][3] = 'A person is clapping.'
    modify_motions[4, :, :, length // 2 - 2:length // 2 + 2] = base_input_motions[11, :, :, 165:169]
    model_kwargs['y']['text'][4] = 'A person is clapping'

    model_kwargs['obs_x0'] = modify_motions
    ...
    torch.save(all_motions,'motion_save.pt')
