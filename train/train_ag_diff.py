...
def init_wandb(config, project_name="default_entity", entity="default_entity", tags=[], notes=None, **kwargs):
    if entity is None:
        assert (
...
def main():
    ...
    print("Training...")
    data_dict=data.dataset.t2m_dataset.data_dict  #[24546,length,263]
    name_list= data.dataset.t2m_dataset.name_list  #[24546]
    ...
    # 使用tqdm包装name_list以显示进度条
    for name in tqdm(name_list, desc="Processing motions"):
        sample= data_dict[name]['motion']  # [length,263]
        if isinstance(sample, np.ndarray):
            sample = sample.T[np.newaxis, :, np.newaxis, :]
        else:
            sample = sample.numpy().T[np.newaxis, :, np.newaxis, :]
        # to tensor
        sample = torch.from_numpy(sample).float()  # [1,263,1,length]
        
        # Recover XYZ *positions* from HumanML3D vector representation: [1,263,1,length]-> [1,22,3,length]
        ...
        sample = model.rot2xyz(x=sample, mask=rot2xyz_mask, pose_rep=rot2xyz_pose_rep, glob=True, translation=True,
                                ...
                                get_rotations_back=False)  #[1,22,3,length]
        data_dict[name]['motion'] = sample.numpy()[0]

    print(f"Dataset size: {len(data_dict)}")

    with open(os.path.join("dataset", "data_dict_xyz.pkl"), "wb") as f:
